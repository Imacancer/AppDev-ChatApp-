...
class MessageController extends ChangeNotifier {
  ...
  Future<void> sendMessage([String? overrideText]) async {
    ...
    if (_selectedMedia != null) {
      ...
      mediaUrl = await uploadMedia(_selectedMedia!);
      if (mediaUrl == null) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['classificationMessages'] != null) {
          ...
          if (classificationMessages.isNotEmpty) {
            final warnings = classificationMessages
                .map((msg) => "🔗 ${msg.url}\n⚠️ ${msg.message}")
                .join("\n\n");

            debugPrint(
              "Caution: The message contains potentially risky links.\n\n$warnings",
            );
          }
        }
